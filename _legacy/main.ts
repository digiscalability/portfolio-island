/**
 * @deprecated This entry point is DEPRECATED. Use main-simple.ts instead.
 *
 * This legacy complex app uses the old full-featured system (Engine, UIManager,
 * full post-processing). It exists for reference only and will be removed in a
 * future sprint. The recommended entry point is main-simple.ts which uses the
 * SimpleApp + GameScene + SimpleRenderer stack.
 *
 * index.html already loads main-simple.ts — do not switch this back.
 */
...
class App {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      if (host === 'localhost' || host === '127.0.0.1' || (window as any).__FORCE_DEBUG) {
        try {
          logger.init();
          console.info('Client logger initialized (overlay + console hook)');
        } catch (_e) {
          console.warn('logger.init failed', _e);
        }
      }
    } catch (_e) {}
    ...
    (async () => {
      try {
        const profile: any = {
          headline: 'Engineer, designer, and creator',
          projects: [],
          hobbies: [],
          timeline: [],
          memories: [],
        ...
        try {
          ...
          if (skipRemoteFetch) {
            ...
            if (r.ok) {
                ...
                for (const lm of ldMatches) {
                  try {
                    ...
                    const collectText = (o: any): string | undefined => {
                      ...
                      if (o.author && typeof o.author === 'object' && o.author.name)
                        res += ' by ' + o.author.name;
                      if (o.description)
                        res +=
                          ' ' +
              ...
              const projectMatches = Array.from(html.matchAll(/case-studies\/(?:[\w-]+)/gi))
                .slice(0, 6)
                .map((m) => m[0].replace('case-studies/', ''));
              ...
              for (const slug of projectMatches) {
                ...
                try {
                  ...
                  const imgs = Array.from(ch.matchAll(/<img[^>]+src=["']([^"']+)["']/gi))
                    .map((mm: any) => mm[1])
                    .filter(Boolean);
                  ...
                  const linkMatches = Array.from(
                    ch.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>/gi),
                  ).map((m: any) => m[1]);
                  const portfolioLinks = linkMatches
                    .filter((u: any) => /portfolio|work|case-studies/gi.test(u))
                    .slice(0, 3);
                  for (const pl of portfolioLinks) {
                    try {
                      ...
                      const pimgs = Array.from(ph.matchAll(/<img[^>]+src=["']([^"']+)["']/gi))
                        .map((mm: any) => mm[1])
                        .filter(Boolean);
                      pimgs.forEach((pi: any) =>
                        imgs.push(/^https?:\/\//i.test(pi) ? pi : new URL(pi, absolute).toString()),
                      );
                    } catch (_e) {
                      /* ignore follow */
                    }
                  ...
                  projects.push({
                    ...
                    url,
                    image: uniq[0] || '/assets/project-thumbnail.svg',
                    gallery: uniq,
                    description: uniq.length ? `Gallery with ${uniq.length} images` : '',
                  });
                } catch (_e) {
                  projects.push({ title: slug.replace(/-/g, ' '), url });
                }
              ...
              // simple team mention
              if (/Abbas/i.test(html))
                profile.timeline.push({ year: 'Now', text: 'Lead Developer @ DigiScalability' });
            }
          }
        } catch (_e) {
          console.warn('digiscalability fetch failed', _e);
        }
        ...
        */
        ...
        console.warn('profile fetch failed', _e);
      }
    })();
    ...
    try {
      ...
      console.error('asset load failed', _e);
      return;
    }
      ...
      if (rendererCtrl) {
        (async () => {
          try {
            ...
            const hdris = (m.textures || []).filter(
              (t: string) => /\.hdr$/i.test(t) && !/^https?:\/\//i.test(t),
            );
                ...
                if (env) {
                  ...
                  if (scene) {
                    ...
                    try {
                      ...
                      /* some three builds don't allow using pmrem as background */
                    }
                  }
                  console.log('Auto-applied HDRI:', first);
                  try {
                    window.dispatchEvent(
                      new CustomEvent('ds:hdri-applied', { detail: { file: first } }),
                    );
                  } catch (_e) {}
                }
              } catch (_e) {
                console.warn('failed to load auto HDRI', first, _e);
              }
            }
          } catch (_err) {
            console.warn('auto HDRI apply failed', _err);
          }
        })();
      }
    } catch (_e) {
      console.warn('initPostProcessing failed', _e);
    }
    ...
    try {
      if (this.engine) this.engine.setControlsEnabled(false);
    } catch (_e) {}
    try {
      if (this.engine) this.engine.setWorldVisible(false);
    } catch (_e) {}
    ...
    try {
      ...
      if (lowEnd) {
        console.log(
          'Low-end device heuristics applied: reducing DPR and disabling heavy postprocessing',
        );
        const rendererCtrl = this.engine?.getRendererController?.();
        try {
          rendererCtrl && rendererCtrl.setPostProcessingEnabled(false);
        } catch (_e) {}
        try {
          (rendererCtrl as any).setSize &&
            (rendererCtrl as any).setSize(window.innerWidth, window.innerHeight);
        } catch (_e) {}
      }
    } catch (_e) {}
    ...
    try {
      (window as any).disablePostProcessing = () => {
        try {
          const rc = (window as any).engine?.getRendererController?.();
          rc && rc.setPostProcessingEnabled(false);
          console.log('Post-processing disabled');
        } catch (_e) {
          console.warn('disablePostProcessing failed', _e);
        }
      };
      (window as any).enablePostProcessing = () => {
        try {
          const rc = (window as any).engine?.getRendererController?.();
          rc && rc.setPostProcessingEnabled(true);
          console.log('Post-processing enabled');
        } catch (_e) {
          console.warn('enablePostProcessing failed', _e);
        }
      };
      (window as any).adjustCamera = (
        off: { x?: number; y?: number; z?: number } | null,
        smooth?: number,
      ) => {
        try {
          ...
          if (typeof smooth === 'number') {
            cc.setSmoothness(smooth);
            console.log('Camera smoothness set', smooth);
          }
        } catch (_e) {
          console.warn('adjustCamera failed', _e);
        }
      };
    } catch (_e) {}
    ...
    try {
      ...
      console.warn('camera tuning panel (dev) failed', _e);
    }
    ...
    try {
      ...
      (window as any).makeHDRIVisible = async (opts: any = {}) => {
        try {
          ...
          scene.traverse((obj: any) => {
            ...
            let mats: any[] = [];
            if (Array.isArray(obj.material)) mats = obj.material;
            else mats = [obj.material];
            mats.forEach((m) => {
              ...
              try {
                if (m && (m.isMeshStandardMaterial || m.isMeshPhysicalMaterial)) {
                  ...
                  try {
                    (window as any).__originalMaterials.push({ obj, material: m });
                  } catch (_e) {}
                  if (typeof m.envMapIntensity === 'number')
                    m.envMapIntensity = Math.max(
                      m.envMapIntensity || 0,
                      opts.envMapIntensity || 0.8,
                    );
                  if (typeof m.metalness === 'number')
                    m.metalness = Math.max(m.metalness || 0, opts.metalness || 0.15);
                  if (typeof m.roughness === 'number')
                    m.roughness = Math.min(
                      typeof m.roughness === 'number' ? m.roughness : 1,
                      opts.roughness || 0.6,
                    );
                  changed.push(m);
                  count++;
                ...
                /* ignore material conversion errors */
              }
            });
          });
          ...
          try {
            ...
            el.className = 'toast';
            el.textContent = 'Visual mode: HDRI boost applied';
          ...
          console.warn('makeHDRIVisible failed', _e);
        }
      };
      (window as any).restoreOriginalMaterials = () => {
        try {
          ...
          const el = document.createElement('div');
          el.className = 'toast';
          el.textContent = 'Visual mode: restored';
          el.style.position = 'fixed';
          el.style.bottom = '18px';
          el.style.left = '18px';
          el.style.zIndex = '2000';
          ...
          console.warn('restoreOriginalMaterials failed', _e);
        }
      };
    } catch (_e) {}
    ...
    const startSequence = async () => {
      ...
      await new Promise<void>((resolve) => {
        const onEnd = () => {
          window.removeEventListener('ui:dialogue-end', onEnd);
          resolve();
        };
      ...
      try {
        ...
        if (cc && typeof cc.setSmoothness === 'function') {
          ...
          const prevOffset = (cc as any).offset
            ? (cc as any).offset.clone()
      ...
      try {
        if (this.engine) this.engine.setControlsEnabled(true);
      } catch (_e) {}
      try {
        if (this.engine) this.engine.setWorldVisible(true);
      } catch (_e) {}
      try {
        if (this.uiManager) this.uiManager.hideWelcomeScreen();
      } catch (_e) {}
      // Fade in UI after camera transition
      try {
        if (this.uiManager) this.uiManager.fadeInUI(900);
      } catch (_e) {}
      try {
        if (this.uiManager) this.uiManager.showToast('You are in control now');
      } catch (_e) {}
    };

    // Start the sequence immediately since PlanetThumbnail is missing
    startSequence().catch((_err) => console.warn('startSequence failed', _err)); // Initialize delivery counter in HUD
    ...
    deliverySystem.setOnDeliveryComplete(() => {
      const completed = deliverySystem.getCompletedCount();
      const tot = deliverySystem.getActiveDeliveries().length + completed;
  ...
  private async loadAssets(onProgress?: (progress: number) => void): Promise<void> {
    ...
    try {
      ...
      console.warn('failed to load manifest', _e);
    }
    ...
    try {
      (window as any).__hdrEnvironments = hdrEnvironments;
    } catch (_e) {
      /* ignore */
    }

    const audios = (manifest.audio || []).map((p: string) => ({
      url: /^https?:\/\//i.test(p) ? p : `/assets/${p}`,
      key: p.replace(/^.*[\\/]/, '').replace(/\.[^/.]+$/, ''),
    }));
    const models = (manifest.models || []).map((p: string) =>
      /^https?:\/\//i.test(p) ? p : `/assets/${p}`,
    );
      ...
      (url: string) =>
        new Promise<void>((resolve) => {
          ...
          loader.load(
            ...
            undefined,
            (_err) => {
              console.warn('texture load failed', url, _err);
              loaded++;
              onProgress && onProgress(loaded / totalItems);
              resolve();
            },
                ...
                try {
                  ...
                  try {
                    ...
                    if (size && size > DEFER_THRESHOLD) {
                      ...
                      try {
                        (window as any).__deferredAudio = (window as any).__deferredAudio || {};
                        (window as any).__deferredAudio[a.key || a.url] = a.url;
                      } catch (_e) {}
                    ...
                    if (!rr.ok) {
                      console.warn('audio fetch not ok', a.url, rr.status);
                      throw new Error('audio fetch not ok');
                    }
                    ...
                    if (typeof am.loadAudioBuffer === 'function') {
                      ...
                      try {
                        await am.loadAudio(a.url, a.key);
                      } catch (_e) {
                        console.warn('audio load fallback failed', a.url, _e);
                        throw _e;
                      }
                    }
                  } catch (valErr) {
                  ...
                  console.warn('audio load failed', a.url, _e);
                }
                loaded++;
                onProgress && onProgress(loaded / totalItems);
                resolve();
        ...
        (url: string) =>
          new Promise<void>((resolve) => {
            gltfLoader.load(
              ...
              undefined,
              (_err: any) => {
                console.warn('model load failed', url, _err);
                loaded++;
                onProgress && onProgress(loaded / totalItems);
                resolve();
              },
  ...
  public static async loadDeferredAudio(keyOrUrl: string) {
    try {
      ...
      console.warn('loadDeferredAudio failed', _e);
    }
  }
  ...
  public static installPointerDebug() {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (el) {
        ...
        overlay.style.left = e.clientX + 12 + 'px';
        overlay.style.top = e.clientY + 12 + 'px';
      } else {
    ...
    try {
      ...
      for (const a of audios) {
        ...
        try {
          const rr = await fetch(url);
          if (!rr.ok) {
            issues.push({ url, status: rr.status });
            continue;
          }
          const ct = rr.headers.get('content-type') || '';
          if (!/audio\//i.test(ct)) {
            issues.push({ url, contentType: ct });
            continue;
          }
        } catch (_e) {
          issues.push({ url, error: _e });
      ...
      if (issues.length) console.warn('manifest validation issues', issues);
      else console.log('manifest validation: OK');
      return issues;
    } catch (_e) {
      console.warn('manifest validation failed', _e);
    }
  }
  ...
  private setupInteractions(): void {
    ...
    interactionSystem.onObjectInteraction((obj: any) => {
      try {
        ...
        const body =
          (obj.userData && (obj.userData.dialogue || obj.userData.bubbleText)) || 'Hello!';
        // compute anchored screen position
        try {
          const cam = (this.engine as any).sceneManager?.getCamera?.();
          const canvas = document.getElementById('game-canvas') as HTMLCanvasElement | null;
          const targetPos =
            obj.mesh && obj.mesh.position
              ? obj.mesh.position
              : obj.position ||
                (obj.getWorldPosition ? obj.getWorldPosition(new THREE.Vector3()) : undefined);
          if (cam && canvas && targetPos) {
            ...
            if (vec.z < 1 && vec.z > -1) {
              ...
              if (typeof this.uiManager.showDialogue === 'function')
                this.uiManager.showDialogue(title, body, x, y - 16);
    ...
    setInterval(() => {
      ...
      if (nearbyZone) {
        ...
        try {
          ...
          const canvas = document.getElementById('game-canvas') as HTMLCanvasElement | null;
          const targetPos =
            nearbyZone.marker && (nearbyZone.marker as any).position
              ? (nearbyZone.marker as any).position
              : nearbyZone.getPosition();
          if (cam && canvas && targetPos) {
            ...
            if (vec.z < 1 && vec.z > -1) {
              ...
              if (lastNearbyZoneId !== nearbyZone.id) {
                this.uiManager?.showSpeechBubble(
                  nearbyZone.name,
                  nearbyZone.description,
                  x,
                  y - 40,
                );
              }
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', async () => {
        ...
        const duration = parseInt(
          (document.getElementById('appointment-duration') as HTMLSelectElement)?.value,
        );
  ...
  public dispose(): void {
    ...
    if (this.engine) {
      try {
        ...
        console.warn('Error disposing engine:', _e);
      }
    ...
    if (this.uiManager) {
      try {
        ...
        console.warn('Error disposing UIManager:', _e);
      }
