...
export class UIManager {
  ...
  public fadeInUI(duration: number = 900) {
    const elements: (HTMLElement | undefined)[] = [
      ...
      this.chatUI?.getElement(),
      this.dialogueUI?.getElement(),
    ];
    elements.forEach((el) => {
      if (el) {
  ...
  private createUI(): void {
    ...
    // Create chat window (owned by ChatUI)
    this.chatUI = new ChatUI(this.container, this.chatSystem);

    // Create dialogue box (owned by DialogueUI)
    this.dialogueUI = new DialogueUI(this.container);
  ...
  // Dialogue helpers — delegated to DialogueUI
  public showDialogue(title: string, text: string, screenX?: number, screenY?: number): void {
    this.dialogueUI.show(title, text, screenX, screenY);
  }
