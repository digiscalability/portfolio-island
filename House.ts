...
export class House {
  ...
  constructor(color: number = 0xffaa66) {
    this.mesh = this.createHouse(color);
  }

  private createHouse(color: number): THREE.Group {
  ...
  const doorMaterial = Materials.createStandardMaterial({ color: 0x654321, metalness: 0.02, roughness: 0.7, envMapIntensity: 0.2 });
  ...
  const windowMaterial = Materials.createStandardMaterial({ color: 0x87ceeb, metalness: 0, roughness: 0.2, envMapIntensity: 0.9 });
  ...
  const trimMat = Materials.createStandardMaterial({ color: 0xffffff, metalness: 0, roughness: 0.6 });
